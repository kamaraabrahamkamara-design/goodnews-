...
def fetch_ledger_data() -> pd.DataFrame:
    ...
    try:
        # FIXED: Replaced 'ascending=False' with 'desc=True' to comply with supabase-py SDK syntax
        response = supabase.table("ledger").select("*").order("created_at", desc=True).execute()
